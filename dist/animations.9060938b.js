...
document.addEventListener('mousemove', function (e) {
  ...
  balls.forEach(function (ball) {
    return ball.style.display = 'grid';
  });
});
...
function animateBall(ballEl, ballSpeed) {
  _gsap.default.set(ballEl, {
    xPercent: -50,
    yPercent: -50
  });
  var ball = document.querySelector(ballEl);
  var pos = {
    x: window.innerWidth / 2,
    y: window.innerHeight / 2
  };
  ...
  _gsap.default.ticker.add(function () {
    var dt = 1.0 - Math.pow(1.0 - speed, _gsap.default.ticker.deltaRatio());
...
var hoverElementTagArray = [
//Enter all els with hover effect
'#logo', '.page-link', '#home_sandbox-button', '#sandbox-return', '.project_title', '.project_info-button', '.project_info-link', '.about-link', '#wakatime-info', '#wakatime-logo', '#form-submit-button', '.section-link', '#footer-link', '.home_link', '#form-success-button', '.about-contact-link'];
hoverElementTagArray.forEach(function (tag) {
  var elements = document.querySelectorAll(tag);
  elements.forEach(function (el) {
    el.addEventListener('mouseover', function (e) {
      document.getElementById('cursor').classList.add('cursor-hover');
      var cursorTrail = Array.from(document.getElementsByClassName('ball'));
      cursorTrail.forEach(function (ball) {
...
function animateAboutTitle() {
  var title = document.getElementById('about-title-2');
  var content = ['Problem Solver', 'Builder', 'Communicator'];
  var currentContent = 0; //current content array index
  var currentChar = 0;
  var currentString = ''; //current string that is rendered without '_'
  var delayFramesTotal = 30; //set how many frames to wait on full word
  var animationSpeed = 50; //set framerate for animation (ms)
  var delayFrames = delayFramesTotal;
  var staggerAnimation = false; //used to half 'typing' animation compared to 'delete' animation
...
document.getElementById('wakatime-info').addEventListener('mouseover', function (e) {
  var hoverEl = document.querySelector('.wakatime-info-hover-el');
  var hoverElRect = hoverEl.getBoundingClientRect();
  var iconRect = document.getElementById('wakatime-info').getBoundingClientRect();
  hoverEl.classList.add('wakatime-info-hover-el-active');
  var x = iconRect.x - hoverElRect.width;
  var y = iconRect.y - hoverElRect.height;
  hoverEl.style.left = "".concat(x, "px");
  hoverEl.style.top = "".concat(y, "px");
});
document.getElementById('wakatime-info').addEventListener('mouseleave', function (e) {
